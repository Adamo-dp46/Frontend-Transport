...
function buildColumns(canEdit: boolean, canDelete: boolean, csrfDelete: string): ColumnDef<Model>[] {
    return [
        {
            accessorKey: "id",
            header: ({ column }) => (
                        ...
                        <DropdownMenuContent align="end">
                            {canEdit && (
                                <DropdownMenuItem asChild>
                                    <a href={`/model/${marque.id}/modifier`}>Modifier</a>
                                </DropdownMenuItem>
                            )}
                            ...
                            {canDelete && (
                                <DropdownMenuItem asChild>
                                    <form
                                        method="POST"
                                        action={`/model/${marque.id}/supprimer`}
                                        onSubmit={(e) => {
                                            if(!confirm("Supprimer ce modèle de pièce ?")) {
                                                e.preventDefault()
                                            }
                                        }}
                                    >
...
export default function ModelTable({models, canEdit, canDelete, csrfDelete}: Props) {
