...
function buildColumns(
    ...
    return [
        ...
        },
        {
            id: "actions",
            cell: ({ row }) => {
                const depannage = row.original
                const enCours = depannage.statut == 'EN COURS'
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                                <a href={`/depannage/${depannage.id}`}>Voir</a>
                            </DropdownMenuItem>
                            ...
                            {canDelete && (
                                <DropdownMenuItem asChild>
                                    <form
                                        method="POST"
                                        action={`/depannage/${depannage.id}/supprimer`}
                                        onSubmit={(e) => {
    ...
    const filters: ServerTableFilter[] = useMemo(() => [
        {
            ...
            name: 'search',
            label: 'Lieu',
            placeholder: 'Lieu de dépannage…',
        },
        {
            type: 'select',
            name: 'car',
            label: 'Véhicule',
            options: cars.map((c) => ({ value: `${c.id}`, label: c.matricule })),
        },
        {
            type: 'date_range',
            name: 'date',
